...
Proof. apply beq_nat_false_iff. Qed.
Lemma N_gt__pos_minus: forall a b: nat, a > b <-> a - b > 0.
Proof. intros; omega. Qed.

Ltac zero := simpl; repeat rewrite <- mult_n_O; repeat rewrite <- plus_n_O; try reflexivity.
Ltac zero_in H := simpl in H; repeat rewrite <- mult_n_O in H; repeat rewrite <- plus_n_O in H; try reflexivity; inversion H.
...
Proof.
  ...
  - induction b. simpl. destruct c.
    ...
    + split. intros. pose proof ((N_minus_plus c (S b)) H) as F. rewrite <- F.
      ...
      rewrite <- N_cons_lt_plus. rewrite N_lt__gt, N_gt__pos_minus in H.
...
Proof.
  ...
  - simpl in IHb. simpl. intros. simpl.
    ...
    assert ((S (S b) - a) + a = S (S b))
      by (apply N_minus_plus; rewrite <- N_gt__pos_minus in H0; apply H0).
    assert (forall n: nat, n + n * (S b) = n * S (S b)).
    { intros. assert (S (S b) = 1 + S b) by reflexivity. rewrite H2, mult_plus_distr_l. omega. }
    repeat rewrite H2. rewrite <- H1. repeat rewrite mult_plus_distr_l.
